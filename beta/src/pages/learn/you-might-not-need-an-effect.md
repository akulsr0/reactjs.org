...
Delete the Effect and instead [adjust the state directly during rendering:](/apis/usestate#storing-information-from-previous-renders)
...
function List({ items }) {
  ...
  if (items !== prevItems) {
    setPrevItems(items);
    setSelection(null);
    return; // Restart rendering immediately
  }
  // ...
}
```
